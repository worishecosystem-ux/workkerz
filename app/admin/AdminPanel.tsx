...
export default function AdminPanel() {
  ...
  */
  ...
  const hasAccess = useCallback(
    (module: AdminModule) =>
      ...
      (superAdmin ||
        (module !== "admins" &&
          canAccessModule(admin.role, roles, module))),
  ...
  */
  ...
  const removeNotification = useCallback(
    (id: string | number) => {
      ...
      setViewOrder((x) =>
        x && String(x.id) === String(id) ? null : x,
      );
    },
    [],
  );
  ...
  */
  ...
  Android WebView:
  When keyboard opens, visualViewport height becomes
  significantly smaller than window.innerHeight.

  Bottom navigation hides while keyboard is visible.
  */
  ...
  useEffect(() => {
    ...
    const checkKeyboard = () => {
      ...
      /*
       * Difference caused by keyboard.
       * 120px threshold prevents normal browser/WebView
       * resizing from triggering the hide.
       */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const roleNames =
          Array.isArray(data.assignedRoles)
            ? data.assignedRoles
                .map((x) => x.name)
                .filter(Boolean)
            : [];
  ...
  const navButton = (
    ...
    return (
      <button
        ...
        className={
          mobile
            ? `flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 px-0.5 text-[9px] font-semibold leading-none ${
                active
                  ? "text-[#FF5C39]"
                  : "text-[#94A3B8]"
              }`
            : `flex min-h-10 w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm font-semibold ${
                active
      ...
      >
        {mobile ? (
          <>
            <div
              className={`flex h-7 w-11 items-center justify-center rounded-xl ${
                active ? "bg-orange-50" : ""
              }`}
            >
  ...
  */
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8FAFC]">
      ...
      <aside
        className={`fixed inset-x-0 bottom-0 z-[60] flex h-[50dvh] w-full flex-col overflow-hidden rounded-t-[24px] border-t border-slate-200 bg-[#F8FAFC] shadow-[0_-15px_40px_rgba(15,23,42,0.15)] transition-transform duration-300 ease-out ${
          drawer
            ? "translate-y-0"
      ...
      >
        <div className="flex h-[60px] shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <span
              className={`shrink-0 rounded-lg px-2 py-1 text-[8px] font-bold ${
                superAdmin
