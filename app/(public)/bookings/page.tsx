...
export default function MyBookingsPage() {
  ...
  const [tab, setTab] = useState<"bookings" | "worker-requests" | "orders">(
    "bookings",
  );
                ...
                <div className="absolute left-0 top-full z-9999 mt-2 w-full overflow-hidden rounded-2xl border border-white/10 bg-[#0f172a] shadow-2xl backdrop-blur-xl">
                  {suggestions.map((item) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (item.type === "request") {
                          ...
                          router.replace("/bookings?tab=worker-requests");
                        } else {
                          setTab("bookings");
                    ...
                    >
                      ...
                      >
                        {item.type === "request" ? "Request" : item.status}
                      </span>
                    </button>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-3 p-4">
              ...
              {filteredWorkerRequests.length === 0 ? (
                ...
                filteredWorkerRequests.map((request) => (
                  ...
                  >
                    {/* TOP LINE */}
                    <div className="h-0.5 bg-linear-to-r from-purple-500 via-violet-500 to-fuchsia-400" />
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span
                              className={`shrink-0 rounded-full px-1.5 py-0.5 text-[8px] font-semibold ${getRequestStatusClass(
                                request.status,
                              )}`}
                            >
                          ...
                          <p className="mt-0.5 truncate text-[9px] text-slate-500">
                            {request.category} · {request.workers_required}{" "}
                            Worker
                            {request.workers_required > 1 ? "s" : ""}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-1.5">
                        <div className="rounded-lg bg-purple-50/70 px-2 py-1.5">
                      ...
                      {/* LOCATION */}
                      <div className="mt-1.5 flex items-center gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5">
                      ...
                      {/* DURATION + BUDGET */}
                      {(request.duration || request.budget !== null) && (
                        <div className="mt-1.5 flex items-center gap-1.5">
                          {request.duration && (
                            <span className="flex items-center gap-1 rounded-full bg-blue-50 px-2 py-1 text-[8px] font-medium text-blue-700">
                          ...
                          {request.project_type && (
                            <span className="truncate rounded-full bg-slate-100 px-2 py-1 text-[8px] font-medium text-slate-600">
                              {request.project_type}
                            </span>
                          )}
                        </div>
                      ...
                      {/* REQUIREMENT */}
                      {request.requirement && (
                        <p className="mt-1.5 truncate rounded-lg bg-slate-50 px-2 py-1.5 text-[9px] text-slate-500">
                          {request.requirement}
                        </p>
                      )}
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <div className="flex min-w-0 flex-1 items-center gap-1.5">
                          ...
                          <span className="truncate text-[9px] text-slate-400">
                            Workkerz Trust · {formatDate(request.created_at)}
                          </span>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            router.push(`/worker-request/${request.id}`)
                          }
                          className="group flex h-8 shrink-0 items-center gap-1 rounded-lg bg-purple-600 px-3 text-[10px] font-bold text-white shadow-sm transition active:scale-[0.98]"
                        >
                          View
